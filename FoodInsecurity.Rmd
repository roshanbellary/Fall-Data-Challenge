...
food_ca <- read_excel("food.xlsx",sheet="CA")
rcols <- c()
for (i in 1:ncol(food_ca)){
  if (any(is.na(food_ca[,i]))){
    rcols <- append(rcols,i)
  }
}
food_ca <- food_ca[-rcols]
for (i in 1:ncol(food_ca)){
  if (any(food_ca[,i]=='NULL')){
    rcols <- append(rcols,i)
  }
}
rcols
...
```{r}
reg_ca<-lm(formula=TractSNAP~Pop2010+LowIncomeTracts+PovertyRate+LA1and10+LATracts1+LATracts10+TractLOWI+TractKids+TractSeniors+TractWhite+TractBlack+TractAsian+TractNHOPI+TractAIAN+TractOMultir+TractHispanic+TractHUNV,data=food_ca)
summary(reg_ca)
nreg_ca<-lm(formula=TractSNAP~TractLOWI+TractKids+TractSeniors+PovertyRate+TractAsian+TractNHOPI+TractAIAN+TractHispanic+TractHUNV+TractBlack+TractWhite,data=food_ca)
summary(nreg_ca)
```
