...
```{r}
anova_food <- food_ca
anova_food[anova_food=="NULL"] <- "0"
select(anova_food, -c(CensusTract,State,County))
```

```{r}
anova <- aov(TractSNAP~TractBlack+TractAsian+TractHispanic+PovertyRate+	
lakids1+lalowi1+LALOWI1_10,data=anova_food)
summary(anova)
```
