...
for (i in 1:length(abbrev)){
  for (var in vars){
    if (any(food[abbrev[i]]$var=="NULL")){
      useless <- append(useless,i)
    }
    if (any(is.null(food[abbrev[i]]$var))){
      useless <- append(useless,i)
    }
  }
}
...
None of the states have null values for the variables in the model!
...
Making Linear Model:
```{r}
nreg_ca<-lm(TractSNAP~TractLOWI+TractKids+TractSeniors+PovertyRate+TractAsian+TractNHOPI+TractAIAN+TractHispanic+TractHUNV+TractBlack+TractWhite,data=food$CA)
summary(nreg_ca)
```

Using model to test on data of remaining states and seeing accuracy:
```{r}
pred <- predict(nreg_ca,food$NY)-food$NY$TractSNAP
boxplot(pred,
main = "Distribution of residuals in predictions from regression",
xlab = "Difference in # of food stamps in census tract",
col = "orange",
border = "brown",
horizontal = TRUE,
notch = TRUE
